...
appname="Defender"                                                                                # The name of our App deployment script
...
if [ $install == "yes" ]; then
    ...
    # Check if app is running, if it is we need to wait, or terminate
    isAppRunning
    
    # Defender sometimes struggles to install without removing the old version first
    if [ -d "/Applications/$app" ]; then
      echo "$(date) | Removing old installation of $appname"
      rm -rf "/Applications/$app"
    fi

    # Wait until no downloads are in progress, otherwise we'll interrup them when we install
    waitForCurl

    # Pause until other installations are finished
    waitForInstaller
