...
reset_office=false
# Remove Entra WorkplaceJoin (MS-Organization-Access) certs from login keychain before enrollment
REMOVE_WORKPLACE_JOIN_CERTS=true
...
# (e.g., while managed by another MDM with Intune device compliance). If not removed, they can block
# fresh Intune enrollment. See removeWorkplaceJoinCerts.sh for a standalone version of this logic.
