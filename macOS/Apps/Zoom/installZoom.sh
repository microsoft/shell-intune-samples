...
processpath="/Applications/zoom.us.app/Contents/MacOS/zoom.us"              # The process name of the App we are installing
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 -L -o "Zoom.pkg" "$weburl"
    if [ $? == 0 ]; then
