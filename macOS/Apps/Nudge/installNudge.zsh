...
processpath="/Applications/Utilities/Nudge.app/Contents/MacOS/Nudge"                  # The process name of the App we are installing
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -o "Nudge.pkg" "$weburl"
    if [[ $? == 0 ]]; then
