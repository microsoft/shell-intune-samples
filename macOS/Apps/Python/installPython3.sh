...
############################################################################################
##
## Script to install the latest [Python3]
## 
## VER 3.0.3
##
## Change Log
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -o "Python3.pkg" "$weburl"
    if [ $? == 0 ]; then
...
function installPKG () {
    ...
    if [ "$?" = "0" ]; then
        ...
        echo "$(date) | Setting default python version"
