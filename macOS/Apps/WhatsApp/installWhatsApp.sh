...
############################################################################################
##
## Script to install the latest [WhatsApp]
## 
## VER 3.0.6
##
## Change Log
...
weburl="https://web.whatsapp.com/desktop/mac/files/WhatsApp.dmg"                                # What is the Azure Blob Storage URL?
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -o "WhatsApp.dmg" "$weburl"
    if [ $? == 0 ]; then
