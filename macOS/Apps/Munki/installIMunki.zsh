...
weburl="https://github.com/munki/munki/releases/download/v5.7.3/munkitools-5.7.3.4444.pkg"      # What is the Azure Blob Storage URL?
weburl_middleware="Path to Munki Middleware"
appname="Munki"                                                                                 # The name of our App deployment script (also used for Octory monitor)
app="Managed Software Center.app"                                                               # The actual name of our App once installed
logandmetadir="/Library/Logs/Microsoft/IntuneScripts/$appname"                                      # The location of our logs and last updated data
processpath="/Applications/$app/Contents/MacOS/Managed Software Center"                         # The process name of the App we are installing
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -o "Munki.pkg" "$weburl"
    if [[ $? == 0 ]]; then
...
function installPKG () {
    ...
    if [ "$?" = "0" ]; then
        ...
        echo "$(date) | Installing Munki Middleware"
        curl "$weburl_middleware" -o /usr/local/munki/middleware_azure.py

        echo "$(date) | Triggering Munki First Check and Update"
