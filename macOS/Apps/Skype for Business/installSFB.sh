...
############################################################################################
##
## Script to download latest Microsoft Skype for Business app for macOS
##
###########################################
...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 -L -o "SkypeForBusiness.pkg" "$weburl"
    if [ $? == 0 ]; then
