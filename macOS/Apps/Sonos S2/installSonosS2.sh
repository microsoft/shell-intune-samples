...
function downloadApp () {
    ...
    cd "$tempdir"
    curl -f -s --connect-timeout 30 --retry 5 --retry-delay 60 --compressed -L -o "Sonos.dmg" "$weburl"
    if [ $? == 0 ]; then
...
function installPKG () {
    ...
    # Check if app is running, if it is we either need to wait or terminate
