...
############################################################################################
##
## Script to install the latest [Teams Classic]
## 
## VER 4.0.5
##
## Change Log
...
appname="Microsoft Teams classic"                                       # The name of our App deployment script
app="Microsoft Teams classic.app"                                       # The actual name of our App once installed
...
function installPKG () {
    ...
    echo "$(date) | Removing any existing files from [$appdir/$app]"
    if [[ -d "$appdir/$app" ]]; then
        rm -rf "$appdir/$app"
    fi
