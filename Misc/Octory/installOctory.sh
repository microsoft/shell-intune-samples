...
## Microsoft, its authors, or anyone else involved in the creation, production, or delivery of the scripts be liable#!/bin/bash
#set -x
...
echo "$(date) | Launching Octory for user [$currentUser]"
sudo -u "$currentUser" Octory.app/Contents/MacOS/Octory -c onboarding.plist
if [[ $? -eq 0 ]]; then
    ...
    sleep 10
    sudo -u "$currentUser" open Octory.app --args -c onboarding.plist
    if [[ $? -eq 0 ]]; then
        echo "$(date) | Octory succesfully launched"
        #cleanup
        exit 0
